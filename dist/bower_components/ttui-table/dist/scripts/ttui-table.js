...
function ttTable($compile, $timeout, tableService) {
	return {
		restrict: 'A',
		scope: false,
		...
		controller: ['$scope', '$element', function($scope, $element) {
			...
			tableCtrl.getColumnData = function(column) {
				...
				angular.forEach(tableCtrl.dropdowns, function(dropdown) {
					if (dropdown.filters && column === dropdown.predicate) {
						setFilters = dropdown;
					}
				});

				if (setFilters) {
					return angular.copy(setFilters.filters);
				} else {
					var data = [];
					angular.forEach(tableCtrl.tableModel, function(obj) {
...
ttTable.$inject = ['$compile', '$timeout', 'tableService'];
	...
	return {
		...
		compile: function(element) {
			...
			return function(scope, element, attributes, controllers) {
				...
				dropdownCtrl.items = [];
				dropdownCtrl.disableAutoFilters = angular.isDefined(attributes.disableAutoFilters) ? true : false;
